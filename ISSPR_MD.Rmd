...
Here we combine the two plots from the between ANOVA (N=11) for original vs. suppressed filtered images
```{r, fig.width=15, fig.height=7}
#Mara COMMENT: Did I have to change it as.numeric here ?; wanna ask before I did a mess again 
subset_ISSPexperiments_bw[ , stim_dur_ms_f := droplevels(stim_dur_ms_f)]
subset_ISSPexperiments_bw_sup[ , stim_dur_ms_f := droplevels(stim_dur_ms_f)]

ggplot()+
  geom_point(data=subset_ISSPexperiments_bw[is.na(common_subj_id)], 
             aes(x = stim_dur_ms_f, y = correct, color = sac_suppression_f, 
                 shape = fixreplay, 
                 group = paste(experiment, sac_suppression_f)))+
  geom_vline(xintercept = 0, linetype = "dotted", alpha = 0.7) + 
  geom_hline(yintercept = 0.5, linetype = "dotted", alpha = 0.7) + 
  geom_line(data=subset_ISSPexperiments_bw[is.na(common_subj_id)], 
             aes(x = stim_dur_ms_f, y = correct, color = sac_suppression_f, 
                 linetype = fixreplay, 
                 group = paste(experiment, sac_suppression_f)), 
            size = 1.5, alpha = 0.8) + 
  labs(x = "Presentation duration [ms]", y = "Proportion scene correctly matched", 
       color = "Simulated suppression", fill = "Simulated suppression", shape = "Task") + 
  geom_point(data= subset_ISSPexperiments_bw_sup[is.na(common_subj_id)], 
             aes(x =stim_dur_ms_f, y = correct, color = sac_suppression_f, 
                 shape = fixreplay, 
                 group = paste(experiment_f, color_yes_f)))+ 
  geom_vline(xintercept = 0, linetype = "dotted", alpha = 0.7) + 
  geom_hline(yintercept = 0.5, linetype = "dotted", alpha = 0.7) + 
  geom_line(data= subset_ISSPexperiments_bw_sup[is.na(common_subj_id)], 
             aes(x = stim_dur_ms_f, y = correct, color = sac_suppression_f, 
                 linetype = fixreplay,
                 group = paste(experiment_f, color_yes_f)), 
            size = 1.5, alpha = 0.8) + 
  ...
  labs(x = "Presentation duration [ms]", y = "Proportion scene correctly matched", 
       color = "Simulated suppression", fill = "Simulated suppression", shape = "Task", 
       linetype = "Task") + 
  facet_wrap(~is.na(common_subj_id))
...
Second plot proportion correct and stim_dur [ms] (N=20)
...
prop_replay_latency_plot <- ggplot(data = p_correct_replay_latency, 
  ...
  scale_x_continuous(breaks = c(-40, -30, -20, -10, 0, 10, 20), limits = c(-40, 20)) +  # X-Achse mit entsprechenden Breaks und Limits
  labs(x = "Display offset re saccade offset [ms]", y = "Proportion scene correctly matched", 
