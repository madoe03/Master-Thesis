...
Here we start with statistic for N=9; Replay (Exp=4) and Exp. 2 with FILTERED condition
```{r}
...
anov_prop_replay_latency 
```
Here I wanna do the histogramms for number of trials and display duration
```{r}
histo_Exp12 <- ggplot(issp12_prop_correct, aes(x = sac_display_off_latency, 
                                     color = stim_dur_ms_f, group = stim_dur_ms_f)) +
  geom_density(aes(fill = stim_dur_ms_f, alpha = stim_dur_ms_f), size = 1, adjust = 1.5) +  # Dichtekurven mit Transparenz
  facet_wrap(~experiment_f, scales = "free_y", ncol = 1) +  # Facetten für Experimente
  scale_color_viridis_d(name = "Display duration [ms]", guide = "none") +  # Farben aus viridis-Skala, ohne doppelte Legende
  scale_fill_viridis_d(name = "Display duration [ms]") +
  scale_alpha_manual(values = seq(0.3, 1, length.out = length(unique(issp12_prop_correct$stim_dur_ms_f))), guide = "none") +
  geom_vline(xintercept = 0, linetype = "dotted", color = "black") +  # Vertikale Linie bei x=0
  scale_x_continuous(name = "Display offset re saccade offset [ms]", limits = c(-60, 60), breaks = c(-60, -30, 0, 30, 60)) +  # Anpassung der x-Achse
...
histo_Exp12
```
Here I wann do the frequency plot for the mean distribution of all samples and observers for replay
no vari. times for all observers, no individual saccade statstics 
```{r}
plot_freq <- ggplot(data = ISSPR_prop_correct_agg, 
                          aes(x=replay_sac_display_off_latency, fill=stim_dur_ms_f, color=stim_dur_ms_f)) + 
  #geom_histogram(binwidth = 3)  +
  geom_freqpoly(binwidth =1 )  +
  #geom_density(alpha= 0.1, adjust = 1/2) + 
  geom_vline(aes(xintercept=0),color="grey", linetype="dashed", size=1) +  
  ocimTheme()+ scale_fill_viridis_d(option = "D", begin = 0, end = 1)+  theme(legend.position = "bottom")+
  labs(x="Display onset- saccade onset [ms]", y="Frequency", 
       fill = "Display duration [ms]", color = "Display duration [ms]")
...
>>>>>>> 225239b58acd0b0ffb4d418caf2d6aa0294dfa26
>>>>>>> ba70bd7b9e574eb2543cba41234be77d82abf2d0
