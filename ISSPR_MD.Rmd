...
#Problem ist, dass ich Proband:innen habe, bei denen für bestimmte Faktorstufen von stim_dur_ms Werte in ‘correct’ fehlen.
# habe hier keine Daten mit common_subj_id == 4 oder common_subj_id == 5 im Datenframe für Experiment 1 & 2
complete( issp12_prop_correct, common_subj_id, stim_dur_ms, fill= list( correct = NA ) )
issp12_prop_correct$stim_dur_ms <-factor(issp12_prop_correct$stim_dur_ms)
issp12_prop_correct$common_subj_id <-factor(issp12_prop_correct$common_subj_id)

anov_issp12<- ezANOVA(data = issp12_prop_correct, 
        dv = .(correct), wid = .(common_subj_id), 
        within = .(stim_dur_ms))


anov_replay<- ezANOVA(data = ISSPR_prop_correct, 
        dv = .(correct), wid = .(common_subj_id), 
        within = .(stim_dur_ms))
...
ezANOVA(data = issp12_prop_correct, 
        wid = .(common_subj_id), dv = .(correct), within = .(stim_dur_ms), between = .(common_subj_id == "all others"))

ezANOVA(data = ISSPR_prop_correct, 
        wid = .(common_subj_id), dv = .(correct), within = .(stim_dur_ms), between = .(common_subj_id == "all others"))
